...
Podemos perceber que o risco mínimo é atingido quando o alpha é `0,29`. Cabe destacar que o ponto ótimo de alpha é aquele que minimiza o risco total esperado, equilibrando ambos os tipos de erro.
...
Como o risco de não lançar é 4,5 vezes maior que o risco de lançar, então a decisão a priori correta seria de `LANÇAR` o medicamento.
...
Como podemos ver, o risco de não lançar o medicamento dado que ele funciona é $991.958,1, enquanto o risco de lançar dado que ele não funciona é $578.705,1. Portanto, a ação ótima a posteriori é `LANÇAR` o medicamento neste cenário. Cabe destacar que, mesmo nesse cenário ruim, na qual a diferença é tão pequena no tamanho do efeito do remédio, como o custo do erro tipo II é muito alto, a decisão será sempre de lançar.
...
Por fim, no cenário 2, o risco de não lançar o medicamento dado que ele funciona é $4.667.526, enquanto o risco de lançar dado que ele não funciona é $29.976,29. Portanto, a ação ótima a posteriori continua sendo `LANÇAR` o medicamento neste cenário. Isso já era esperado, pois o tamanho do efeito é maior que o do cenário 1, e o custo do erro tipo II é muito alto.

Cabe destacar que, se a diferença entre os custos fossem menores, talvez a decisão entre os cenários fossem diferentes. Por exemplo, se o custo do erro tipo II fosse menor, poderíamos ter uma situação onde a decisão ótima fosse lançar o medicamento no cenário 1, onde a diferença entre o tamanho do efeito era menor, e não lançar no cenário 2, onde o tamanho do efeito é maior. Isso mostra como a modelagem de crenças a priori e os custos dos erros influenciam as decisões.
